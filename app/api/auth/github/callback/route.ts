...
export async function GET(req: Request) {
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.json({ error: 'GitHub credentials not configured on backend' }, { status: 500 });
  }
  ...
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      if (githubPicture && userRecord.picture !== githubPicture) {
        await db.update(users)
          .set({ picture: githubPicture })
