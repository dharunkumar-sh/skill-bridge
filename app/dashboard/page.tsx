...
import { CheckCircle2, TrendingUp, Zap, Target, BookOpen, Clock, Lightbulb, AlertTriangle, Compass, Loader2 } from "lucide-react";
...
export default function OverviewPage() {
  ...
  const [overdueCourses, setOverdueCourses] = useState<any[]>([]);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (profileRes.ok) {
        ...
        setAiData(typeof analysis === "string" ? JSON.parse(analysis) : analysis);
      }
  ...
  const maxWeeks = aiData?.skillGapAnalysis
    ? Math.max(...aiData.skillGapAnalysis.map((g: any) => g.estimatedWeeks || 1), 4)
    : 4;
        ...
        <div className="bg-midnight-900 border border-white/5 rounded-3xl p-6 shadow-xl">
          ...
          <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-2">
            {enrolledCourses.slice(0, 8).map((course, idx) => {
              ...
              return (
                <div key={idx} className={`shrink-0 w-56 p-4 rounded-2xl border ${isCompleted ? "bg-emerald-950/20 border-emerald-500/20" : isOverdue ? "bg-rose-950/20 border-rose-500/20" : "bg-midnight-950/50 border-white/5"}`}>
                  {course.courseThumbnail && (
                    <img src={course.courseThumbnail} alt="" className="w-full h-24 object-cover rounded-xl mb-3" />
                  )}
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                      ...
                      {isCompleted ? "✓ Done" : isOverdue ? "⏰ Overdue" : course.status}
                    </span>
                  </div>
                </div>
              );
        ...
        <div className="lg:col-span-1 bg-midnight-900 border border-white/5 rounded-3xl p-6 shadow-xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto hide-scrollbar space-y-5 max-h-[220px]">
            {!aiData?.skillGapAnalysis ? (
              ...
              aiData.skillGapAnalysis.map((gap: any, idx: number) => {
        ...
        <div className="bg-midnight-900 border border-white/5 rounded-3xl p-6 md:p-8 shadow-xl">
          ...
          <div className="relative pt-2 pb-4">
            ...
            <div className="flex gap-4 overflow-x-auto hide-scrollbar snap-x pb-4">
              {aiData.milestones.map((ms: any, idx: number) => (
