...
import { Home, BookOpen, Users, Briefcase, Settings, Rocket, FileText } from "lucide-react";
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 z-40 bg-midnight-950/80 backdrop-blur-sm lg:hidden"
          onClick={onClose}
        />
      )}

      {/* Sidebar Container */}
      <aside className={`fixed top-0 left-0 h-screen z-50 w-64 bg-midnight-950/95 backdrop-blur-xl border-r border-white/5 transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex flex-col h-full">
            ...
            {SIDEBAR_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className={isActive ? "text-blue-100 font-semibold" : "text-slate-400 group-hover:text-slate-200"}>
                    {item.name}
                  </span>
                </Link>
              );
          ...
          <div className="p-4 border-t border-white/5">
            <div className="bg-linear-to-br from-midnight-800 to-midnight-900 border border-midnight-700 p-4 rounded-xl text-center shadow-lg relative overflow-hidden group">
              ...
              <p className="text-slate-400 text-xs mb-3">Get 1-on-1 expert mentorship.</p>
              <button className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors shadow-md">
                Upgrade Now
              </button>
            </div>
          </div>
